    ...
    try {
        con.getConnection((err, con) => {
            if (err) {
                ...
                const query1 = "INSERT INTO guest(name, contact, email, address, gender, age, occupation, company, room, advance, referral) VALUES (?)"
                con.query(query1, [Object.values(req.body)], (error) => {
                    if (error) throw error;
                    // const query = `INSERT INTO user(name, contact, email, pass, role) VALUES(?)`;
                    // con.query(query, [], (error, result) => {
                    //     if (error) throw error;
                        const query = `UPDATE rooms SET occupied = 'Y' WHERE name = ?`;
                        con.query(query, req.body.room, (error, result) => {
                            if (error) throw error;
                            return res.send('success')
                        })
                    })
    ...
    try {
        con.getConnection((err, con) => {
            if (err) {
                ...
                const query1 = "INSERT INTO staff(name, contact, email, address, gender, age, department, referral) VALUES (?)"
                con.query(query1, [Object.values(req.body)], (error) => {
                    if (error) throw error;
                    return res.send('success')
                })
